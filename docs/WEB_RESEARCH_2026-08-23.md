...
| [Sailing World: Progress to Perfect Tacks](https://www.sailingworld.com/how-to/progress-to-perfect-tacks/) | オシレーティングな風では平均風向よりヘダーになったときのタックが基本だが、タック回数、風圧、パフ／ラルも含めて判断する。理由のないタックを増やさないことも重視する | 「振れ始めなら必ず返す」と固定しない。ヘダーを走る時間の利益と操作ロスを同じ海面で比較し、差が小さければ風圧・相手・マークへ判断を戻す |
| [Sailing World: Why Wind Shifts Matter More Than You Think](https://www.sailingworld.com/racing/why-wind-shifts-matter-more/) | ヘダーを見ても即座にタックするとは限らず、艇の反応、風の強さ、振れの大きさを合わせて判断する | 振れを確認した直後と最大振れを別試走にし、420教育モデルのタック回復を含めて差を出す |
| [Speed & Smarts Issue 100](https://hyse.org/pdf/speedandsmarts/SS-Issue-100.pdf) | オシレーティングな風では中央値を基準にヘダーへ入ったら返す考え方を示し、最大振れまで走り続けるとヘダーを長く走る危険を指摘する | 最大振れまで待つ案も見せるが、待つ間の相対ゲインを隠さない。比較時刻をそろえ、結果と考え方を同時に示す |
| [ILCA UK: Tactics Made Simple](https://ilca.uk/news/tactics-made-simple-by-jon-emmett) | 操作ロスの小さい艇は小さな振れにも反応しやすく、操作ロスの大きい艇は振れが確かになるまで待つ必要があると説明する | 正解時刻を艇種共通にしない。420向けの暫定ロスを計算へ含め、実艇データで未校正であることを画面に残す |
...
| [Black & Wiliam: Assessment and Classroom Learning](https://www.gla.ac.uk/t4/learningandteaching/files/PGCTHE/BlackandWiliam1998.pdf) | 形成的評価では、学習者から得た記録を解釈し、次の学習活動を調整することが重要 | リプレイを説明だけで終えず、操作記録に応じて同条件の再試行か別の風への応用を選ぶ |
...
1. 目的が分かる5つの420向けコーチドリルを、自由設定より先に置く。
...
21. 直近8試行の練習記録を端末内だけに保存し、同条件で根拠とマーク到達を2回確認できた場合だけ、風の変化を変えた応用へ進む。
22. 振れを観測できる最初の1秒後と最大振れで最初の操作だけを変え、共通時刻の対艇ゲインと操作ロスを比較する。リフトなら時刻比較の前に走り続ける判断を返す。
...
- 420の実艇で、振れ始めから最大振れまでの風向、タック後4秒の速度回復、相手との相対位置を記録し、教育モデルの「振れ始め／最大振れ」差を校正できるか
